...
def main():
    ...
    if args.pretrained_model_name_or_path is not None:
        model = AutoencoderKL.from_pretrained(args.pretrained_model_name_or_path).to(device=accelerator.device,dtype=args.mixed_precision)
        model.requires_grad_(False)
        model.post_quant_conv.requires_grad_(True)
        model.decoder.requires_grad_(True)

    else:
        assert False,"please input sd vae's pretrained_model_name_or_path"
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                vqmodel.save_pretrained(os.path.join(output_dir, "AutoEncoderKL"))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            fmap = model.module.encode(pixel_values).latent_dist.sample()
            fmap = model.module.decode(fmap).sample
